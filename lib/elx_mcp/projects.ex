defmodule ElxMcp.Projects do
  ...
  def status_summary(%Scope{project_id: project_id} = scope, opts \\ []) do
    ...
    %{
      ...
      recent: recent_items(scope, recent_limit),
      in_review:
        list_tickets(scope, status: "in_review", limit: recent_limit) |> Enum.map(&summarize/1)
    }
  end
  ...
  defp recent_items(%Scope{} = scope, limit) do
    epics = list_epics(scope, limit: limit) |> Enum.map(&summarize/1)
    stories = list_user_stories(scope, limit: limit) |> Enum.map(&summarize/1)
    tickets = list_tickets(scope, limit: limit) |> Enum.map(&summarize/1)
  ...
  defp summarize(%Epic{} = e),
    do: %{type: "epic", key: e.key, title: e.title, status: e.status, updated_at: e.updated_at}

  defp summarize(%UserStory{} = s),
    do: %{
      type: "user_story",
      key: s.key,
      title: s.title,
  ...
  defp summarize(%Ticket{} = t),
    do: %{type: "ticket", key: t.key, title: t.title, status: t.status, updated_at: t.updated_at}
